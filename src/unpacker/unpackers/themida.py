"""
Themida unpacker: uses Unipacker (emulation-based) when available.

Themida is not in Unipacker's known packer list; Unipacker will treat it as unknown
...
class ThemidaUnpacker(BaseUnpacker):
    """Themida unpacking via Unipacker (emulation; unknown packer mode)."""
    ...
    def unpack(self, sample_path: Path, options: UnpackOptions) -> UnpackResult:
        out_path = options.output_dir / f"{sample_path.stem}.unpacked.themida{sample_path.suffix}"
        ...
        return run_unipacker_emulation(
            ...
            out_path,
            options,
            packer_label="unipacker_themida",
        )
